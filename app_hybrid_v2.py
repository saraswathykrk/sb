...
def fetch_from_vedabase(canto, chapter, verse):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Look for "Devanagari" section (original Sanskrit script)
            devanagari_match = re.search(r'Devanagari\s*\n(.+?)(?=\nVerse text|\nSynonyms|$)', full_text, re.DOTALL)
            if devanagari_match:
                devanagari_verse = devanagari_match.group(1).strip()
                print(f"✅ Found Devanagari verse: {devanagari_verse[:50]}...")
            
            # Look for "Verse text" section (romanized Sanskrit)
            ...
            if devanagari_verse or sanskrit_verse or translation:
                save_to_database(canto, chapter, verse, devanagari_verse, sanskrit_verse, 
            ...
            return {
                'devanagari_verse': devanagari_verse,
...
def get_from_database(canto, chapter, verse):
    ...
    try:
        ...
        if result:
            ...
            return {
                'devanagari_verse': result[0],
                'sanskrit_verse': result[1],
                'word_meanings': result[2],
...
def fetch_verse_hybrid(canto, chapter, verse):
    ...
    if db_result:
        return {
            'success': True,
            'reference': verse_ref,
            'devanagari_verse': db_result['devanagari_verse'],
    ...
    if web_result:
        return {
            'success': True,
            'reference': verse_ref,
            'devanagari_verse': web_result['devanagari_verse'],
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("🕉️  HYBRID Srimad Bhagavatam Verse Finder (COMPLETE)")
    print("="*70)
    print("📊 Features:")
    print("  • Devanagari Sanskrit (original script)")
    print("  • Romanized Sanskrit (transliteration)")
    print("  • Word-for-word meanings (synonyms)")
    print("  • English translation")
    print("  • Complete purport")
    print("  • Database caching for instant access")
    print("="*70)
    print()
